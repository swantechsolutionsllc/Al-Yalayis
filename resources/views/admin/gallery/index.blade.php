            ...
            <div class="col-lg-12">
                ...
                <div class="block block-rounded">
                    ...
                    <ul class="nav nav-tabs nav-tabs-block">
                        <li class="">
                            <a class="nav-link @if(Request::is('gallery')) active @endif" href="{{url('gallery')}}">Images</a>
                        </li>
                        <li class="">
                            <a class="nav-link" href="{{url('video-gallery')}}">Videos</a>
                        </li>
                ...
                <div class="row items-push  img-fluid-100">
                    @foreach($images as $image)
                        <div class="col-sm-6 col-md-2 animated fadeIn">
                            <a class="img-link img-link-zoom-in img-thumb img-lightbox" style="width:100%;" data-name="{{$image->name}}" data-size="{{$image->size}}" data-url="{{$image->url}}" data-uploaded="{{$image->created_at}}" data-dimessions="{{$image->dimessions}}"  href="{{ asset($image->url)}}">
                                <div>  <span class="spans float-left"><strong> Name :</span> </strong><span class="spans float-right"> {{ strlen($image->name)>15? substr($image->name, 0,15).'..':$image->name}}</span></div>
                                <img class="img-fluid" src="{{ asset($image->thamnail_url)}}" alt="">
                                <div> <strong> <span class="spans float-left"> Size :</span></strong><span class="spans float-right"> {{ $image->size}}</span></div>
                                
                            </a>
                            <input type="checkbox" name="ids[]" value="{{$image->id}}" class="check-boxes ">
                        </div>
                        @endforeach
       ...
       <div class="info-div">
            <table class="table table-striped table-bordered">
                ...
                <tr>
                    ...
                    <td   class="spans2"><span id="file-url" style="font-size:10px;font-family:monospace;"  ></span></td>
                </tr>
